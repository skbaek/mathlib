...
lemma hov_succ_succ (k m : nat) (h0 : m + 1 < k) : 
  ∀ a : term, hov k (a.subst $ swap m) → hov k a 
| (# n) h1 := 
  begin
...
| (a & (b & c)) h1 := 
  begin
    cases h1,
    { left, exact hov_succ_succ a h1 },
    right, apply hov_succ_succ (b &c) h1
  end
  
lemma hov_succ (k : nat) :
  ∀ a : term, hov k (a.subst $ swap k) → hov (k + 1) a  
| (# m) h0 := 
  begin
...
| (a & (b & c)) h0 := 
  begin
    cases h0,
    { left, apply hov_succ a h0 },
    right, apply hov_succ (b & c) h0
  end
...
lemma holds_iff_holds_of_eq_except : 
  ...
  eq_except M N k → fov k p → (M ⊨ p ↔ N ⊨ p) 
| M N k ⟪b, a⟫ h0 h1 := 
...
lemma zero_fov_swap_aux : 
  ∀ k : nat, ∀ p : form, fov (k + 1) p → 
    fov k (form.subst [(k, (# (k + 1)) & (# k)), (k + 1, # k)] p) 
| k ⟪b, a⟫           h0 := h0 ∘ (hov_succ k _)
| k (form.bin b p q) h0 := 
  by { cases h0, constructor; 
       apply zero_fov_swap_aux; assumption }
| k (form.qua b p)   h0 := zero_fov_swap_aux (k + 1) p h0

lemma zero_fov_swap {p : form} :
  fov 1 p → fov 0 (p.swap 0) := 
zero_fov_swap_aux 0 _

lemma fov_swap : 
  ∀ k m : nat, m + 1 < k → 
  ∀ p : form, fov k p → fov k (p.swap m)
| k m h0 ⟪b, a⟫           h1 := h1 ∘ (hov_succ_succ k _ h0 _)
...
lemma fov_swap_many (ae : bool) :
  ∀ {p : form} {k : nat}, 
  fov (k + 1) p → fov k (swap_many ae p) 
| ⟪b, a⟫           := λ _, id
...
lemma fov_swap_all (ae : bool) :
  ∀ {k : nat} {p : form}, fov k p → fov k (swap_all ae p)  
| k ⟪b, a⟫           h0 := h0
| k (form.bin b p q) h0 := 
